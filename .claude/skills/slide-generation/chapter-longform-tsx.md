...
```bash
# Windows Git Bash (uv가 PATH에 없는 경우)
.venv/Scripts/python.exe scripts/regenerate_slides.py $PROJECT
...
> ⚠️ **렌더 실패 시 TSX 자동 삭제**: 렌더가 RuntimeError로 실패하면 파이프라인이 해당 tsx 파일을 삭제한다.
> 렌더 실패 후 tsx가 없어졌다면 재작성 필요. 이 단계(pre-render)에서 잡아야 파이프라인 cascade를 막을 수 있다.
